    ...
    <div class="w-full h-full bg-gray-800 bg-opacity-90 top-0 overflow-y-auto overflow-x-hidden fixed sticky-0 z-10"
        ...
        <div class="w-full absolute z-10 right-0 h-full overflow-x-hidden transform translate-x-0 transition ease-in-out duration-700"
            ...
            <div class="2xl:w-4/12 bg-gray-50 h-screen overflow-y-hidden hover:overflow-scroll p-8 absolute right-0">
                ...
                <!--nội dung comment-->
                <div class="w-full p-3 mt-8 bg-white rounded flex" v-for="cmt in comments">
                    ...
                    <article class=" mb-6 text-base bg-white rounded-lg ">
                        ...
                        <div class="reply" v-for="reply in replys"> 
                            ...
                            <article class=" mb-6 ml-6 mt-2 text-base bg-white rounded-lg " v-if="isShowReply && cmt.id == reply.reply_id">
                        <footer class="flex justify-between items-center mb-2">
                            <div class="flex items-center">
                                <p class="inline-flex items-center mr-3 text-sm text-gray-900 "><img
                                        class="mr-2 w-6 h-6 rounded-full" :src="reply.user.avatar">{{ reply.user.fullname }}</p>
                                <p class="text-sm text-gray-600 dark:text-gray-400">{{ formatDate(reply.createdAt) }}</p>
                            </div>
                            <!--menu edit comment cấp 1-->
                            ...
                            <div id="dropdownComment1" v-if="isShowEditLV1 && idComment == reply.id"
                                ...
                                <ul class="py-1 text-sm text-gray-700 dark:text-gray-200"
                                    ...
                                    <li>
                                        <a href="#"
                                            class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"
                                            @click="oncloseEdit(); sendCMT(reply); !showEditLV1(reply.id)">Chỉnh sửa</a>
                                    </li>
                                    <li>
                                        <a href="#"
                                            class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"
                                            @click="deleteCmt(reply.id)">Xóa</a>
                                    </li>
        ...
        <div class=" bg-white w-11/12 md:max-w-md mx-auto rounded shadow-lg z-50 overflow-y-auto ">
            ...
            <div class="px-4">
                <div class="flex items-center mt-2">
                    <img class="w-6 h-6 rounded-full mr-2" :src="user.avatar" alt="Avatar">
                    <textarea class="w-full px-3 py-2 text-gray-700 border rounded-lg focus:outline-none text-sm"
                        type="text" v-model="comment" v-on:keyup.enter="replycomment()"
                        placeholder="Thêm bình luận..."></textarea>
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        async replycomment() {
            const result = await blogService.replyCMTBlog(this.comment, this.blogId, this.user.id, this.idComment);
            if (result.status == 200) {
                this.oncloseReply()
                blogService.getComment(this.blogId).then((data) => { this.comments = data })
                this.$refs.toast.showToast(result.data.message)
