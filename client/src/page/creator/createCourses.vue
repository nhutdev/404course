        ...
        <div class=" bg-white w-11/12 md:max-w-3xl mx-auto rounded shadow-lg z-50 overflow-y-auto max-h-full">
            ...
            <div class="course" v-if="isShowCourse">
                <div class="flex flex-row py-2 px-4 bg-violet-300">
                    <h5 class="text-lg font-semibold flex-grow text-white">Thêm khóa học</h5>
                ...
                <div class="modal-footer">
                    <div class="py-4 px-4">
                        <label for="text" class="block mb-2 text-base font-medium text-gray-900 dark:text-white">Tiêu
                            đề</label>
                ...
                <div class="modal-footer py-3 px-4">
                    ...
                    <button class="py-2 px-4 bg-violet-300 text-white rounded-lg cursor-pointer" @click="addCourse()">Bước
                        tiếp theo</button>
                </div>
            </div>
            ...
            <div class="index_view" v-if="isShowIndex">
                <div class="flex flex-row py-2 px-4 bg-violet-300">
                    <h5 class="text-lg font-semibold flex-grow text-white">Thêm chỉ mục</h5>
                    <div class="flex space-x-6 ">
                        <span class="cursor-pointer px-4 py-2 bg-violet-500 rounded" @click="oncloseAddIndex()">+ Thêm chỉ
                            mục</span>
                    </div>
                    <button @click="onclose()" type="button"
                        class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                    ...
                    <div class="py-4 px-4">
                        <div class="overflow-x-auto bg-white rounded-lg shadow cursor-pointer">
                            <table class="w-full whitespace-no-wrap bg-white overflow-hidden table-striped">
                                <thead>
                                    <tr class="text-left ">
                                        <th class="px-6 py-3 text-gray-500 font-bold tracking-wider uppercase text-xs">STT
                                        </th>
                                        <th class="px-6 py-3 text-gray-500 font-bold tracking-wider uppercase text-xs">Tiêu
                                            đề</th>
                                ...
                                <tbody class="text-sm">
                                    ...
                                    <tr class="focus-within:bg-gray-200 overflow-hidden"
                                        v-for="(index_course, index) in index_courses" :key="index">
                                        <td class="border-t">
                ...
                <div class="modal-footer py-3 px-4">
                    <button class="py-2 px-4 bg-violet-300 text-white rounded-lg cursor-pointer" @click="addAllIndex()">Bước
                        tiếp theo</button>
                </div>
            </div>
            ...
            <div class="index_view" v-if="isShowContent">
                ...
                <div class="modal-footer py-3 px-4">
                    <button class="py-2 px-4 bg-violet-300 text-white rounded-lg cursor-pointer"
                        @click="addAllContent()">Hoàn
                        thành</button>
                </div>
            </div>
    ...
    <div class=" fixed w-full h-full top-0 left-0 flex items-center justify-center z-50 " v-show="isShowAddContent">
        ...
        <div class=" bg-white w-11/12 md:max-w-md mx-auto rounded shadow-lg z-50 overflow-y-auto ">
            ...
            <div class="px-4">
                <div>
                    ...
                    <select id="select" name="select" v-model="id_index"
                        ...
                        <option v-for="index in index_courses" :key="index.id" :value="index.id">{{ index.title_index }}
                        </option>
                    </select>
                </div>
            ...
            <div class="py-3 px-4">
                <button
                    class="  py-2 px-4 bg-gradient-to-r from-indigo-100 via-purple-300 to-pink-200 text-white rounded-lg cursor-pointer mr-4"
                    @click="addContent()">Thêm nội dung</button>
                <button
                    class="  py-2 px-4 bg-gradient-to-r from-indigo-100 via-purple-300 to-pink-200 text-white rounded-lg cursor-pointer"
                    @click="oncloseAddContent()">Đóng</button>
            </div>
        </div>
    </div>
    ...
    methods: {
        ...
        addAllIndex() {
            if (this.index_courses.length > 0) {
                ...
                this.$refs.toast.showToast('Vui lòng nhập ít nhất 1 chỉ mục để qua bước tiếp theo.')
            }
        ...
        async addContent() {
            try {
                const result = await this.$axios.post(`course/index/content/add/${this.id_index}`, {
